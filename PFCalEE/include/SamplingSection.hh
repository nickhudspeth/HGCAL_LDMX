...
public:
    ...
    SamplingSection(std::vector<std::pair <G4double, std::string>> iEle)
    {
        ...
        std::vector<G4double> aThicknessVec;
        std::vector<std::string> aMaterialVec;
    ...
    /* Alternate constructor for use with imported CAD Models */
    SamplingSection(std::vector<std::tuple <G4double, std::string, std::string>> iEle)
    {
        ...
        for (unsigned i = 0; i < iEle.size(); i++)
        {
            aThicknessVec.push_back(std::get<0>iEle.at(i));
            aMaterialVec.push_back(std::get<1>iEle.at(i));
            filenames.push_back(std::get<2>iEle.at(i));
        }
        Total_thick = 0;
        n_sens_elements = 0;
        n_elements = 0;
    ...
    std::vector<std::string> filenames; /* Filename field for use when importing CAD models */
};
